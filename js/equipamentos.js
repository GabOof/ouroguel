...
let paginaAtualEquipamentos = 1;
const EQUIPAMENTOS_POR_PAGINA = 8;
...
function marcarStatus(status) {
    const radio = document.querySelector(`input[name="status"][value="${status || "disponivel"}"]`);
...
async function salvarEquipamento(event) {
    ...
    try {
        if (equipamentoEditando) {
            await equipamentosService.atualizar(equipamentoEditando, dadosEquipamento);
        ...
        mostrarMensagem("Erro", "Erro ao salvar equipamento: " + error.message, "error");
    }
}
...
function obterEquipamentosFiltrados() {
    ...
    return equipamentos.filter((equipamento) => {
        ...
        const temPreco = valorHora > 0 || valorDia > 0 || valorMes > 0;
        ...
        const textoEquipamento = [
            ...
            equipamento.quantidadeAlugada,
            status.texto,
        ]
            .join(" ")
            .toLowerCase();

        return textoEquipamento.includes(termo);
    });
}
...
function renderizarTabelaEquipamentos() {
    ...
    const totalPaginas = Math.ceil(equipamentosFiltrados.length / EQUIPAMENTOS_POR_PAGINA);

    if (paginaAtualEquipamentos > totalPaginas) {
        paginaAtualEquipamentos = totalPaginas;
    }

    const inicio = (paginaAtualEquipamentos - 1) * EQUIPAMENTOS_POR_PAGINA;
    const fim = inicio + EQUIPAMENTOS_POR_PAGINA;
    const equipamentosDaPagina = equipamentosFiltrados.slice(inicio, fim);

    equipamentosList.innerHTML = equipamentosDaPagina
        .map((equipamento) => {
            const status = obterStatusVisual(equipamento);
    ...
    renderizarPaginacaoEquipamentos(equipamentosFiltrados.length);
}

function renderizarPaginacaoEquipamentos(totalEquipamentos) {
    const paginacao = document.getElementById("equipamentosPaginacao");

    if (!paginacao) {
        return;
    }

    const totalPaginas = Math.ceil(totalEquipamentos / EQUIPAMENTOS_POR_PAGINA);
    ...
    const inicio = (paginaAtualEquipamentos - 1) * EQUIPAMENTOS_POR_PAGINA + 1;

    const fim = Math.min(paginaAtualEquipamentos * EQUIPAMENTOS_POR_PAGINA, totalEquipamentos);
...
function mudarPaginaEquipamentos(novaPagina) {
    const totalEquipamentos = obterEquipamentosFiltrados().length;
    const totalPaginas = Math.ceil(totalEquipamentos / EQUIPAMENTOS_POR_PAGINA);
    ...
    paginaAtualEquipamentos = novaPagina;
    renderizarTabelaEquipamentos();
}

window.mudarPaginaEquipamentos = mudarPaginaEquipamentos;
...
async function editarEquipamento(equipamentoId) {
    try {
        const tituloModal = document.getElementById("modalEquipamentoTitulo");

        if (tituloModal) {
            tituloModal.textContent = "Editar Equipamento";
        }
        ...
        const botaoSalvar = document.querySelector('#equipamentoForm button[type="submit"]');

        if (botaoSalvar) {
            botaoSalvar.innerHTML = '<i class="fas fa-sync-alt"></i> Atualizar Equipamento';
        }
        ...
        mostrarMensagem("Erro", "Não foi possível carregar o equipamento para edição.", "error");
    }
}
...
async function excluirEquipamento(equipamentoId) {
    ...
    try {
        ...
        mostrarMensagem("Erro", "Erro ao excluir equipamento: " + error.message, "error");
    }
}
...
function limparFormularioEquipamento() {
    ...
    if (tituloModal) {
        tituloModal.textContent = "Novo Equipamento";
    }
    ...
    if (nomeEquipamento && modal?.classList.contains("active")) {
